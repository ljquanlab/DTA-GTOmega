...
for epoch in range(epo_num):
    ...
    with torch.no_grad():
        ...
        if np.mean(val_loss) < val_loss_record:
            ...
            torch.save(model.state_dict(), f'saved_pth/Davis_repr/{split_strategy}_model_copy.pth')
        else:
            patient -= 1
            if patient == 0:break
...
serial_check_l = [i for i in range(8)]

if serial_check_l == -1:
    model.load_state_dict(torch.load(f'saved_pth/Davis_repr/{split_strategy}_model_copy.pth',map_location='cpu'))
else:
    for serial_check in serial_check_l:
        print(f'Check model:{serial_check}')
        model.load_state_dict(torch.load(f'saved_pth/Davis_repr/{split_strategy}_model_{serial_check}.pth',map_location='cpu'))
        ...
        np.save(f'./record_new/Davis_repr/{split_strategy}_Y.npy', Y)
        np.save(f'./record_new/Davis_repr/{split_strategy}_{serial_check}.npy', P)
        ...
        pcc = pear(Y, P)
        sp = spear(Y, P)
        with open(f'./record_new/Davis_repr/{split_strategy}.txt','a') as f:
            f.write('--------------------------------------------------------------------------\n')
