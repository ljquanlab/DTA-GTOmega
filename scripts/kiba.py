...
for epoch in range(epo_num):
    ...
    with torch.no_grad():
        ...
        if np.mean(val_loss) < val_loss_record:
            ...
            torch.save(model.state_dict(), f'saved_pth/KIBA_repr/{split_strategy}_model.pth')
        else:
            patient -= 1
            if patient == 0:break
...
serial_check = -1
if serial_check == -1:
    model.load_state_dict(torch.load(f'saved_pth/KIBA_repr/{split_strategy}_model.pth',map_location='cpu'))
else:
    model.load_state_dict(torch.load(f'saved_pth/KIBA_repr/{split_strategy}_model_{serial_check}.pth',map_location='cpu'))
...
pcc = pear(Y, P)
sp = spear(Y, P)
print(f'KIBA {split_strategy}, label shape is {Y.shape}, pred shape is {P.shape}')
print('MSE:', m, MSE)
# print('R2:', r2)
print('CI:', CI)
print('PCC:', pcc[0])
...
S = P
P = [0 if i<12.1 else 1 for i in S]
Y = [0 if i<12.1 else 1 for i in Y]
