...
with open(f'data/{dataset}/compound_to_smiles_TDC.pkl','rb') as f:
    df_drugs_smiles = pickle.load(f)

all_smiles = list(df_drugs_smiles.values())
...
if split_strategy == 'cold_target' or split_strategy == 'cold_drug_target':
    ...
    print('Train Set has proteins number:',len(set(t_train)))
    print('Validation Set has proteins number:',len(set(t_val)))
...
import torch
import json
with open('config/Train_KIBA_repr.json') as f:
    params = json.load(f)
...
split_strategy, len(train_loader), len(val_loader), len(test_loader), TOTAL_ATOM_FEATS
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode='min', factor=0.98, patience=4, min_lr=1e-4, verbose=True)
...
for epoch in range(epo_num):
    ...
    with torch.no_grad():
        ...
        if np.mean(val_loss) < val_loss_record:
            ...
            torch.save(model.state_dict(), f'saved_pth/BindingDB_repr/{split_strategy}_model_copy1.pth')
        else:
            patient -= 1
            if patient == 0:break
...
serial_check_l = [i for i in range(13)]
if serial_check_l == -1:
    model.load_state_dict(torch.load(f'saved_pth/BindingDB_repr/{split_strategy}_model_copy1.pth',map_location='cpu'))
else:
    for serial_check in serial_check_l:
        model.load_state_dict(torch.load(f'saved_pth/BindingDB_repr/{split_strategy}_model_{serial_check}.pth',map_location='cpu'))
        ...
        with torch.no_grad():
            ...
            logger.info(f'Test shape: {Y.shape} {P.shape}')
        np.save(f'./record_new/BindingDB_repr/{split_strategy}_Y.npy', Y)
        np.save(f'./record_new/BindingDB_repr/{split_strategy}_{serial_check}.npy', P)
        ...
        pcc = pear(Y, P)
        sp = spear(Y, P)
        with open(f'./record_new/BindingDB_repr/{split_strategy}.txt','a') as f:
            f.write('--------------------------------------------------------------------------\n')
            f.write(f'BindingDB {split_strategy}, label shape is {Y.shape}, pred shape is {P.shape} \n')
            f.write(f'MSE:, {MSE}\n')
            # print('R2:', r2)
            f.write(f'CI:, {CI}\n')
            f.write(f'PCC:, {pcc[0]}\n')
            f.write(f'Spear:, {sp[0]}\n')
            ...
            f.write(f'Precision:, {Precision_dev}\n')
            f.write(f'Reacll:, {Reacll_dev}\n')
            f.write(f'Accuracy:, {Accuracy_dev}\n')
            f.write(f'F1 score:, {f1}\n')
            f.write(f'AUC:, {AUC_dev}, {ROC_AUC}\n')
            f.write(f'PRC:, {PRC_dev}, {PRC_AUC}\n')
            f.write(f'MCC:, {mcc}\n')
            f.write('--------------------------------------------------------------------------\n')
